...
# Created by write_sdc on Tue Sep 30 11:28:46 2025
...
create_generated_clock [get_pins U_Clock_Gating/GATED_CLK]  -name ALU_CLK  -source [get_ports REF_CLK]  -master_clock REF_CLK  -divide_by 1  -add
...
group_path -name INOUT  -from [list [get_ports UART_RX_IN] [get_ports REF_CLK] [get_ports UART_CLK] [get_ports RST_N]]  -to [list [get_ports UART_TX_O] [get_ports parity_error] [get_ports framing_error]]
group_path -name INREG  -from [list [get_ports UART_RX_IN] [get_ports REF_CLK] [get_ports UART_CLK] [get_ports RST_N]]
group_path -name REGOUT  -to [list [get_ports UART_TX_O] [get_ports parity_error] [get_ports framing_error]]
set_input_delay -clock RX_CLK  54.252  [get_ports UART_RX_IN]
set_output_delay -clock TX_CLK  1736.06  [get_ports UART_TX_O]
